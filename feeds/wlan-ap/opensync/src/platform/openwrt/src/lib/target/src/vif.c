...
static struct vif_crypto *vif_get_vif_crypto_map(struct blob_attr **tb)
{
...
int vif_load_wds_sta_config(struct blob_buf *b)
{
	FILE *fd;
	struct stat st;
	char *jsonTxt;
...
extern ovsdb_table_t table_Manager;
static struct blob_buf bBkup= { };
static struct blob_buf bActive= { };

int vif_apply_wds_sta_config( void )
{
	...
	if (false == ovsdb_table_select_one(&table_Manager, SCHEMA_COLUMN(Manager, is_connected), "true", &manager))
	{
		/* Manager reports no connection to the cloud.
		   Apply the saved wds-sta config to attempt recovery */
		if (vif_load_wds_sta_config(&bBkup)) {
			return -1;
		}
		...
		if (uci_section_to_blob(uci, "wireless", blobmsg_get_string(tb[WIF_ATTR_IFNAME]), &bActive, &wifi_iface_param)) {
			LOGI("WDS backhaul back-up file overwriting existing SSID");
		} else {
			LOGI("WDS backhaul back-up file creates new SSID");
		}
